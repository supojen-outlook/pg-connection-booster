...
confirm_install() {
    ...
    echo -e ""
    echo -e "  ${GREEN}【第二階段：SSL 設定】${NC}"
    echo -e "  7. ${GREEN}07-setup-ssl.sh${NC}            - 設定 SSL/TLS"
    echo -e ""
    echo -e "  ${GREEN}【第三階段：系統服務】${NC}"
    echo -e "  8. ${GREEN}08-create-systemd-service.sh${NC} - 建立 systemd 服務"
    echo -e ""
    echo -e "  ${GREEN}【第四階段：資料庫角色建立】${NC}"
    echo -e "  9. ${GREEN}09-create-pgbouncer-role.sh${NC} - 建立 pgBouncer 管理角色"
    echo -e " 10. ${GREEN}10-create-app-database.sh${NC}   - 建立應用程式資料庫"
    echo -e ""
    echo -e "  ${GREEN}【第五階段：設定檔與啟動】${NC}"
    echo -e " 11. ${GREEN}11-create-config.sh${NC}        - 建立設定檔（使用已存在的角色 hash）"
    echo -e " 12. ${GREEN}12-start-service.sh${NC}        - 啟動服務"
    echo -e " 13. ${GREEN}13-verify-installation.sh${NC}  - 驗證安裝"
    echo ""
