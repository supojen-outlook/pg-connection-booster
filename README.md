...
### **管理腳本 9：show-connection.sh - 顯示資料庫連線資訊**

**目的：** 顯示如何連線到 pgBouncer 和 PostgreSQL，包含各種客戶端的連線字串

**具體操作：**
- 讀取 `.env` 中的連線設定（資料庫名稱、角色、密碼、埠號等）
- 自動偵測本機 IP 位址
- 顯示基本連線資訊（資料庫、角色、pgBouncer 位址）
- 顯示 **psql** 連線指令範例（含密碼方式）
- 顯示 **.NET Core** 連線字串：
  - `appsettings.json` 格式
  - `Program.cs` 註冊範例（使用 Npgsql）
- 顯示其他語言的連線字串：
  - **Python** (psycopg2)
  - **Node.js** (pg)
  - **Go** (pgx)
...
**為什麼需要：** 方便開發者快速取得正確的連線資訊，避免手動拼湊連線字串出錯
